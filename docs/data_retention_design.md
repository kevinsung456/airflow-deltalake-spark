...
## 8) FAQ - "메타DB 입력 시 DAG이 동적으로 생성되는가?"

짧게 답하면 **"기본 설계는 DAG 정의를 동적으로 '생성'하기보다, 단일 DAG가 메타DB를 읽어 동적으로 '실행 대상'을 결정"**하는 방식입니다.

### 8.1 권장 기본 패턴: Single DAG + Dynamic Task Mapping

- Airflow DAG 파일(`data_retention_registry_dag.py`)은 1개를 고정 배포
- DAG 실행 시점마다 메타DB(`retention_policy`)에서 활성 정책을 조회
- 조회된 정책 리스트를 기반으로 테이블별 삭제/이관 태스크를 런타임에 fan-out
- 즉, **DAG 수가 늘어나는 방식이 아니라 태스크 인스턴스 수가 실행마다 동적으로 변함**

장점:

- 운영 단순성(코드/배포/권한 관리 단순)
- UI/모니터링 일관성(하나의 DAG에서 추적)
- 정책 변경 반영이 즉시 가능(다음 스케줄부터 적용)

### 8.2 대안 패턴: Meta-driven DAG Factory (정말 DAG을 여러 개 생성)

- 메타DB를 파싱해 Python import 시점에 DAG 객체를 N개 생성
- 예: 팀/도메인별로 서로 다른 DAG ID를 자동 생성

주의점:

- DAG 파싱 부하 증가(정책 수 많으면 스케줄러 부담)
- DAG ID lifecycle 관리 필요(정책 삭제/비활성화 시 잔존 DAG 처리)
- Airflow 배포 파이프라인과 파싱 안정성 고려 필수

### 8.3 구현 예시 (Airflow 2.3+ Dynamic Task Mapping)
...
def retention_dag():
    ...
    def load_active_policies() -> list[dict]:
        """메타DB에서 활성 정책 목록 조회."""
        # return [{"db_name":"sales", "table_name":"orders", "retention_days":30, ...}, ...]
        return []

    @task
    def run_one_policy(policy: dict, retention_mode: str = "hard_delete"):
        """정책 1건 실행 (Spark submit or API trigger)."""
        pass

    policies = load_active_policies()
    run_one_policy.expand(policy=policies)
...
질문하신 방식 중 실무 권장은 다음과 같습니다.

- **권장:** 메타DB에 `table/column/retention_days` 입력 → **동일 DAG**이 이를 읽고 **동적 태스크 매핑**으로 실행
- **비권장(특수 케이스만):** 메타DB 입력 건마다 **별도 DAG 객체를 자동 생성**

즉, "메타DB 입력 시 dynamic하게 변하는 것"은 맞지만, 보통은 **DAG 자체가 아니라 DAG 내부의 실행 태스크 집합이 동적**으로 변합니다.
